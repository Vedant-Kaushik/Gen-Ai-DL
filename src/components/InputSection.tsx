      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div className="relative">
          <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
            <Link className={`w-5 h-5 ${isUrlValid ? 'text-dark-400' : 'text-red-500'}`} />
          </div>
          <input
            type="url"
            value={url}
        ...
        <div className="flex flex-wrap gap-2">
          {[
            ...
            <button
              ...
              className="text-xs px-3 py-1 bg-dark-200 hover:bg-dark-300 rounded-full text-dark-600 transition-colors duration-200"
            >
              {example}
            </button>
          ))}
